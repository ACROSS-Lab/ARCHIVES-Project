/**
* Name: Hanoi ABM Flood Model 1926 (V5-optimized)
* Description:
...
*
* PERFORMANCE NOTES (optimized variant, 2026-06-13). Behaviour and numerics are intended to
*   match "Hanoi ABM Flood Model 1926.gaml"; only speed-only changes were applied (no formula or
*   hydraulic constant touched):
*     A) flow_step phase B uses a guarded loop over the flow list instead of building a
*        fresh `flow_list where each.has_out` sub-list every sub-step - one fewer heap allocation per
*        sub-step; non-donor cells (has_out = false) skip the body, identical result.
*     B) flow_step phase A avoids repeated neighbour terrain lookups by using precomputed face slots.
*     C) the static overlay layers `river` and `lake` are drawn with `refresh: false` (real water is
*        painted on the `cell` grid below) - GUI-only, no effect on compute or the batch experiment.
*     D) flow_step phase A uses a plain `loop over: flow_list` with an `if (!idle_hr)` guard instead
*        of allocating `flow_list where (!each.idle_hr)` every sub-step.
*     E) hourly activity rebuild uses a tracked `stirred_cells` list instead of scanning every
*        active cell to find cells that moved or received water last hour.
*     F) visual color refresh can be disabled (`enable_visuals = false`) for batch/headless runs.
*        River-cell water is display-only; weir hydraulics use the observed stage directly.
*     G) lake settling exits early only when a settle sub-step moves exactly zero water.
*     H) batch sensitivity results are emitted once per finished simulation instead of every cycle.
*     I) display-only base-cell and building colors are skipped when visuals are disabled.
*     J) unused per-cell `nb_n` and `is_breached` state fields were removed.
*     K) quiet hours skip hydraulic sub-steps when no cells were stirred and no gates are open.
*     L) per-face `max(z, nb.z)` is precomputed as `zmax_face` for the hydraulic hot loop.
*     M) `n_breached` is maintained incrementally on breach/reseal instead of counted hourly.
*     N) `refresh_visuals` is a full no-op when `enable_visuals = false`.
*     O) phase A uses a plain loop instead of `ask ... parallel`, avoiding per-sub-step agent
*        scheduling while keeping all water mutations in phase B.
*     P) phase B uses plain `loop over: flow_list` instead of sequential `ask`, avoiding agent-context overhead.
*     Q) the 4-neighbour face loop is unrolled into scalar face slots (`nb0..nb3`, `out0..out3`).
*     R) hourly storage/flooded metrics are computed in one active-cell loop.
*     S) each breached gate scans receiver cells once instead of evaluating `with_min_of` and
*        `with_max_of` separately.
*/

model HanoiABMFlood1926CV5
...
global {
	...
	bool   enable_visuals <- true;            // false skips map color updates for batch/headless runs
	...
	list<cell> stirred_cells <- [];           // cells that moved/received water and seed next hour's flow_list
	...
	bool  result_written <- false;            // batch output guard
	...
	init {
		...
		ask cell {
			...
			loop nb over: neigh_open {
				...
				zmax_face << max(z, nb.z);
			}
			if (nb_open > 0) { nb0 <- neigh_open[0]; k0 <- K_face[0]; zmax0 <- zmax_face[0]; }
			if (nb_open > 1) { nb1 <- neigh_open[1]; k1 <- K_face[1]; zmax1 <- zmax_face[1]; }
			if (nb_open > 2) { nb2 <- neigh_open[2]; k2 <- K_face[2]; zmax2 <- zmax_face[2]; }
			if (nb_open > 3) { nb3 <- neigh_open[3]; k3 <- K_face[3]; zmax3 <- zmax_face[3]; }
		}
		...
		loop times: settle_iters {
			moved_V_sub <- 0.0;
			do flow_step dts: 120.0;
			if (moved_V_sub <= 0.0) { break; }
		}
	...
	// ================================================================== water engine
	// Phase A: every wet cell proposes outflow volumes to lower neighbours.
	...
	action flow_step (float dts) {
		// Phase A: proposals only. No water_h mutation happens here, so a plain loop preserves
		// the two-phase scheme while avoiding per-sub-step `ask` scheduling overhead.
		loop c over: flow_list {
			c.has_out <- false;
			if (!c.idle_hr) {
				c.out0 <- 0.0; c.out1 <- 0.0; c.out2 <- 0.0; c.out3 <- 0.0;
				float totV <- 0.0;
				float max_dW <- 0.0;
				if (c.water_h > EPS_WET) {
					float wsA <- c.z + c.water_h;
					if (c.nb_open > 0) {
						float dW0 <- wsA - (c.nb0.z + c.nb0.water_h);
						if (dW0 > EPS_HEAD) {
							if (dW0 > max_dW) { max_dW <- dW0; }
							float heff0 <- wsA - c.zmax0;
							if (heff0 > EPS_WET) {
								float v0 <- min((heff0 ^ 1.6667) * sqrt(dW0) * c.k0 * dts, dW0 * cap_vol);
								if (v0 > 0.0) { c.out0 <- v0; totV <- totV + v0; }
							}
						}
					}
					if (c.nb_open > 1) {
						float dW1 <- wsA - (c.nb1.z + c.nb1.water_h);
						if (dW1 > EPS_HEAD) {
							if (dW1 > max_dW) { max_dW <- dW1; }
							float heff1 <- wsA - c.zmax1;
							if (heff1 > EPS_WET) {
								float v1 <- min((heff1 ^ 1.6667) * sqrt(dW1) * c.k1 * dts, dW1 * cap_vol);
								if (v1 > 0.0) { c.out1 <- v1; totV <- totV + v1; }
							}
						}
					}
					if (c.nb_open > 2) {
						float dW2 <- wsA - (c.nb2.z + c.nb2.water_h);
						if (dW2 > EPS_HEAD) {
							if (dW2 > max_dW) { max_dW <- dW2; }
							float heff2 <- wsA - c.zmax2;
							if (heff2 > EPS_WET) {
								float v2 <- min((heff2 ^ 1.6667) * sqrt(dW2) * c.k2 * dts, dW2 * cap_vol);
								if (v2 > 0.0) { c.out2 <- v2; totV <- totV + v2; }
							}
						}
					}
					if (c.nb_open > 3) {
						float dW3 <- wsA - (c.nb3.z + c.nb3.water_h);
						if (dW3 > EPS_HEAD) {
							if (dW3 > max_dW) { max_dW <- dW3; }
							float heff3 <- wsA - c.zmax3;
							if (heff3 > EPS_WET) {
								float v3 <- min((heff3 ^ 1.6667) * sqrt(dW3) * c.k3 * dts, dW3 * cap_vol);
								if (v3 > 0.0) { c.out3 <- v3; totV <- totV + v3; }
							}
						}
					}
				}
				if (totV > 0.0) {
					float availV <- min(c.water_h * cell_a, max_dW * cap_vol);
					if (totV > availV) {
						float sc <- availV / totV;
						c.out0 <- c.out0 * sc; c.out1 <- c.out1 * sc; c.out2 <- c.out2 * sc; c.out3 <- c.out3 * sc;
					}
					c.has_out <- true;
					c.idle_subs <- 0;
				} else if (idle_sleep) {
					c.idle_subs <- c.idle_subs + 1;
					if (c.idle_subs >= 2) { c.idle_hr <- true; }
				}
			}
		}

		// Phase B (sequential push): plain loop avoids per-agent `ask` overhead in every sub-step.
		loop c over: flow_list {
			if (c.has_out) {
				float tot <- 0.0;
				if (c.out0 > 0.0) {
					cell to0 <- c.nb0;
					to0.water_h <- to0.water_h + c.out0 * inv_cell_a;
					tot <- tot + c.out0;
					if (!to0.stirred) { to0.stirred <- true; stirred_cells << to0; }
					to0.idle_hr <- false; to0.idle_subs <- 0;
					if (!to0.in_active) { to0.in_active <- true; pending_cells << to0; }
					if (!to0.in_flow) { to0.in_flow <- true; pending_flow << to0; }
				}
				if (c.out1 > 0.0) {
					cell to1 <- c.nb1;
					to1.water_h <- to1.water_h + c.out1 * inv_cell_a;
					tot <- tot + c.out1;
					if (!to1.stirred) { to1.stirred <- true; stirred_cells << to1; }
					to1.idle_hr <- false; to1.idle_subs <- 0;
					if (!to1.in_active) { to1.in_active <- true; pending_cells << to1; }
					if (!to1.in_flow) { to1.in_flow <- true; pending_flow << to1; }
				}
				if (c.out2 > 0.0) {
					cell to2 <- c.nb2;
					to2.water_h <- to2.water_h + c.out2 * inv_cell_a;
					tot <- tot + c.out2;
					if (!to2.stirred) { to2.stirred <- true; stirred_cells << to2; }
					to2.idle_hr <- false; to2.idle_subs <- 0;
					if (!to2.in_active) { to2.in_active <- true; pending_cells << to2; }
					if (!to2.in_flow) { to2.in_flow <- true; pending_flow << to2; }
				}
				if (c.out3 > 0.0) {
					cell to3 <- c.nb3;
					to3.water_h <- to3.water_h + c.out3 * inv_cell_a;
					tot <- tot + c.out3;
					if (!to3.stirred) { to3.stirred <- true; stirred_cells << to3; }
					to3.idle_hr <- false; to3.idle_subs <- 0;
					if (!to3.in_active) { to3.in_active <- true; pending_cells << to3; }
					if (!to3.in_flow) { to3.in_flow <- true; pending_flow << to3; }
				}
				c.water_h <- max(0.0, c.water_h - tot * inv_cell_a);
				moved_V_sub <- moved_V_sub + tot;
				if (!c.stirred) { c.stirred <- true; stirred_cells << c; }
			}
		}
	...
	reflex hydraulics {
		...
		list<cell> prev_stirred <- list(stirred_cells);
		list<cell> seeds <- prev_stirred where each.in_active;
		if (!empty(prev_stirred)) { ask prev_stirred { stirred <- false; } }
		stirred_cells <- [];
		flow_list <- [];
		loop c over: seeds {
		...
		if (!empty(flow_list) or !empty(breached_cells)) {
	...
	action weir_step (float dts) {
		...
		loop bc over: breached_cells {
			...
			if (!empty(cands)) {
				...
				float wl <- 0.0;
				float wl2 <- 0.0;
				loop cand over: cands {
					float wc <- cand.z + cand.water_h;
					if (rc = nil or wc < wl) { rc <- cand; wl <- wc; }
					if (sc_ = nil or wc > wl2) { sc_ <- cand; wl2 <- wc; }
				}
				if (se > wl + EPS_HEAD) {                            // river -> floodplain
					...
					if (hu > 0.0) {
						...
						if (v > 0.0) {
							...
							if (!rc.stirred) { rc.stirred <- true; stirred_cells << rc; }
							rc.idle_hr <- false;
							rc.idle_subs <- 0;
					...
					if (wl2 > se + EPS_HEAD and sc_.water_h > EPS_WET) {
						...
						if (hu > 0.0) {
							...
							if (v > 0.0) {
								...
								if (!sc_.stirred) { sc_.stirred <- true; stirred_cells << sc_; }
								sc_.idle_hr <- false;
								sc_.idle_subs <- 0;
	...
	reflex hourly_tick when: current_date >= next_tick {
		...
		loop b over: building {
			b.depth_w <- b.my_cell = nil ? 0.0 : max(0.0, b.my_cell.water_h - b.my_cell.water0);
			b.status <- b.depth_w > flood_thr ? 2 : (b.depth_w > wet_thr ? 1 : 0);
			if (b.status = 1) { n_bldg_wet <- n_bldg_wet + 1; }
			if (b.status = 2) { n_bldg_flooded <- n_bldg_flooded + 1; }
			if (enable_visuals) { b.color <- b.status = 2 ? #red : (b.status = 1 ? #orange : rgb(120, 120, 125)); }
		}

		// indicators & exact ledger (only weir gates connect river and floodplain)
		stored_V <- 0.0;
		int n_flood_cells <- 0;
		loop c over: active_cells {
			stored_V <- stored_V + c.water_h * cell_a;
			if (c.water_h - c.water0 > wet_thr) { n_flood_cells <- n_flood_cells + 1; }
		}
		float expected <- lake_init_V + river_in_V - river_out_V;
		mass_err_pct <- stored_V = 0.0 ? 0.0 : 100.0 * (stored_V - expected) / max(1.0, expected);
		flooded_km2 <- n_flood_cells * cell_a / 1e6;
...
species dyke schedules: dyke where each.breakable {
	...
	reflex check_reseal when: state_s = "BREACHED" and reseal_time != nil and current_date >= reseal_time {
		state_s <- "RESEALED";
		breached_cells <- breached_cells - my_cells;
		n_breached <- n_breached > 0 ? n_breached - 1 : 0;
...
grid cell file: grid_file("../includes/" + dem_name) neighbors: 4
	...
	list<float> zmax_face <- [];           // per-open-face max(z, nb.z), static terrain term
	cell nb0 <- nil;
	cell nb1 <- nil;
	cell nb2 <- nil;
	cell nb3 <- nil;
...
experiment flood_1926 type: gui {
	...
	parameter "Update map colors" var: enable_visuals category: "Output";
...
experiment sensitivity type: batch repeat: 1 keep_seed: true until: sim_finished {
	...
	parameter "Update map colors" var: enable_visuals among: [false];
	//method exhaustive;

	reflex results {
		ask simulations where (each.sim_finished and !each.result_written) {
			result_written <- true;
