/**
* Name: Phu Tho FastBathtub Flood Model (Discharge-driven)
* Author: Thành Đô Nguyễn (2026-06-25)
*
* A fork of "Phu Tho FastBathtub Flood Model.gaml" that DROPS the river-stage-file
...
* the discharge file (WaterDischarge_PhuTho_hourly.csv) through the Manning rating
* curve, capped at stage_cap. If the discharge record is missing the stage holds at
* base_stage.
*
* WARNING (inherited from the parent): the Phú Thọ discharge file is a TIME-SYNCED
* PLACEHOLDER (a synthetic 0->3000 ramp re-dated onto the event window), not a real
* Sep-2024 hydrograph. Since this model has NO measured-stage fallback, the run is
* only as trustworthy as that discharge series - replace it with a real hydrograph
...
* SRTM DEM, the river polygon stage boundary, the channel baseflow, the buildings,
* the observation points and the per-step GeoTIFF export.
*
* INPUTS (../includes/ and ../includes/phu-tho/).
*   DEM: dem-phutho-epsg-3857.tif (metric).
*   River: water_phutho.shp (Thao/Red-River water polygon) - the stage boundary.
*   Buildings: building_phutho.shp (clipped to the domain in init).
*   Forcing: WaterDischarge_PhuTho_hourly.csv -> Manning rating curve -> river stage.
*   NOTE: PhuThoStage2024_hourly.csv is deliberately NOT used.
*/
model PhuThoFastBathtubFloodDischarge
...
global {
	...
	// ------------------------------------------------------------------ river stage forcing
	// DISCHARGE-DRIVEN ONLY (no measured-gauge stage file). The discharge file
	// (PLACEHOLDER - see the header warning) is pushed through the Manning rating
	// curve; anchors q_min -> base_stage, q_max -> peak_stage. There is NO upper
	// bound on the rating curve except the hard stage_cap ceiling. If the discharge
	// record is missing the stage holds at base_stage.
	...
	// (Mountainous reach: the channel sits high in the SRTM; a POSITIVE offset of
	//  ~+5..+7 m lifts the gauge band onto the floodplain. See the parent model.)
	...
	string export_dir <- "../exported_results/phutho_fastbathtub_discharge/";

	init {
		write "=== Phu Tho FastBathtub Flood (river-seeded, DISCHARGE-driven stage) ===";
		...
		if cell_dx < 1.0 {
			write "WARNING: cell_dx = " + cell_dx + " (looks like DEGREES, not metres). GAMA did NOT "
				+ "project the DEM to a metric CRS - reproject the DEM before trusting any area/celerity.";
		}
		...
		// --- discharge record (the SOLE driver; PLACEHOLDER - dates are M/D/YYYY H:MM) -
		...
		if empty(q_values) {
			...
			write "WaterDischarge_PhuTho_hourly.csv empty -> stage holds at base_stage (no stage-file fallback)";
		} else {
			q_min <- min(q_values); q_max <- max(q_values);
			write "Discharge (driver, PLACEHOLDER): " + length(q_values) + " values, " + first(q_dates) + ".." + last(q_dates)
				+ ", " + (q_min with_precision 1) + "-" + (q_max with_precision 1) + " m3/s"
				+ "  (WARNING: synthetic placeholder - replace with a real Sep-2024 hydrograph)";
		}
	...
	// ====================================================================== forcing
	// placeholder discharge interpolated in time
	...
	// discharge -> power-law rating curve -> stage: the SOLE forcing. No measured-gauge
	// stage file. The curve has no upper bound (discharge fully drives the height)
	// except the hard stage_cap. If the discharge record is missing, hold at base_stage.
	float stage_at (date d) {
		...
		return min(stage_cap, s);   // hard ceiling
	}
...
//  Experiments
// ==========================================================================
experiment phutho_fastbathtub_discharge type: gui {
	parameter "Use placeholder discharge (WaterDischarge_PhuTho_hourly.csv)" var: use_discharge_csv category: "Forcing";
...
experiment phutho_fastbathtub_discharge_fast type: gui {
	parameter "Use placeholder discharge (WaterDischarge_PhuTho_hourly.csv)" var: use_discharge_csv category: "Forcing";
	parameter "Rating curve exponent" var: rating_exponent category: "Forcing";
	parameter "Peak river stage (m)" var: peak_stage category: "Forcing";
	parameter "Stage cap (m) - hard ceiling" var: stage_cap min: 5.0 max: 60.0 category: "Forcing";
