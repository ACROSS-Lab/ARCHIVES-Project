/**
* Name: Hanoi FastFlood Model LISEM OpenLISEM Optimized V4
* Author: Thành Đô Nguyễn
*
* V4 of the OPTIMIZED variant of "Hanoi FastFlood Model LISEM OpenLISEM.gaml".
* On top of O1-O24 of Optimized / V2 / V3 (kept unchanged, summarized below),
* V4 adds five optimizations and a production experiment:
*
*   O25 [exact] River-interior cells removed from relaxation pass 1 in
*       forcing mode. Since pass 2 skips river cells (O21), the flux a river
*       cell exports is only ever consumed by the gathers of its LAND
*       neighbours. A river cell with no land neighbour ("river interior" -
*       the bulk of the channel) feeds only river cells: its q_* has no
*       reader at all. Pass 1 therefore runs on land + bank-river cells only
*       (bank = river cell with >= 1 non-river neighbour; dyke cells count
*       as land). River blocks are wet and active for the entire run, so
*       this removes the largest constant cost of quiet hours (pre-breach,
*       late recession), where the active set is almost pure river. The
*       static refinement (source mode) keeps the full grid: there pass 2
*       visits every cell, so every flux has a reader. River-interior q_*
*       stays exactly 0 in forcing mode (never written), which strengthens
*       the O17 invariant "inactive => q = 0".
*   O26 [exact] Boundary-cell asks are skipped when their list is empty.
*       The flood is interior for most of the run, so the active boundary
*       list is usually empty: 2 parallel dispatches per iteration saved.
*   O27 [exact] The hourly block-wetness / stale-flux scan (O17/O18) runs as
*       a parallel ask instead of a sequential loop. The wet <- true writes
*       are idempotent (same benign-race pattern as the pass-2 activation
*       writes); the q/had_q resets touch only the cell's own attributes.
*   O28 [exact] rebuild_active_lists marks blocks with an 'active' flag and
*       collects members in a single pass over the blocks - no
*       remove_duplicates, no temporary concatenated block lists.
*   O29 [init]  The b-shape fixed-point closure of the static pipeline
*       breaks early on convergence (|b_new - b| < 0.001), as the C++
*       reference does (do_break in rastersuperfastflow.h); init-only, and
*       b can differ from the fixed-15-iterations value by < 0.001.
*   ... and the display-free experiment fastflood_1926_fast for production /
*       calibration runs: no grid raster redraw every cycle (time-series
*       chart and monitors only), identical simulation state and outputs.
*
* Inherited optimizations (full details in the Optimized / V2 / V3 headers;
* all [exact] except O6):
...
*/
model HanoiFastFloodLISEMOpenLISEMOptimizedV4
...
global {
	...
	init {
		write "=== Hanoi FastFlood (LISEM / OpenLISEM port, OPTIMIZED V4) ===";
...
species flow_block schedules: [] {
	int bid;
	bool wet <- false;
	bool active <- false;   // O28: marked by rebuild_active_lists (wet or wet-adjacent)
