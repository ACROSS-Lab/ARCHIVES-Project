/**
* Name: Hanoi ABM Flood Model 1926 (V4-optimized)
* Description:
...
*     A) flow_step phase B uses a guarded loop over the awake sub-step list instead of building a
...
*     D) flow_step phase A builds `awake_flow` with an `if (!idle_hr)` guard instead of allocating
*        `flow_list where (!each.idle_hr)` every sub-step.
...
*     O) sub-steps build `awake_flow` from `flow_list` order, so sleeping cells skip phase A/B
*        without changing the sequential donor order used by the original engine.
*     P) phase B uses plain `loop over` instead of sequential `ask`, avoiding agent-context overhead.
...
global {
	...
	list<cell> awake_flow <- [];              // non-sleeping subset visited by the next sub-step
	...
	action flow_step (float dts) {
		// Phase A (parallel): awake cells propose outflow volumes. Sleeping cells are not visited
		// again inside the hour unless a neighbour or gate pushes water into them.
		awake_flow <- [];
		loop c over: flow_list {
			if (!c.idle_hr) { awake_flow << c; }
		}
		ask awake_flow parallel: true {
			has_out <- false;
			out0 <- 0.0; out1 <- 0.0; out2 <- 0.0; out3 <- 0.0;
			float totV <- 0.0;
			float max_dW <- 0.0;
			if (!idle_hr and water_h > EPS_WET) {
				float wsA <- z + water_h;
				if (nb_open > 0) {
					float dW0 <- wsA - (nb0.z + nb0.water_h);
					if (dW0 > EPS_HEAD) {
						if (dW0 > max_dW) { max_dW <- dW0; }
						float heff0 <- wsA - zmax0;
						if (heff0 > EPS_WET) {
							float v0 <- min((heff0 ^ 1.6667) * sqrt(dW0) * k0 * dts, dW0 * cap_vol);
							if (v0 > 0.0) { out0 <- v0; totV <- totV + v0; }
						}
					}
				}
				if (nb_open > 1) {
					float dW1 <- wsA - (nb1.z + nb1.water_h);
					if (dW1 > EPS_HEAD) {
						if (dW1 > max_dW) { max_dW <- dW1; }
						float heff1 <- wsA - zmax1;
						if (heff1 > EPS_WET) {
							float v1 <- min((heff1 ^ 1.6667) * sqrt(dW1) * k1 * dts, dW1 * cap_vol);
							if (v1 > 0.0) { out1 <- v1; totV <- totV + v1; }
						}
					}
				}
				if (nb_open > 2) {
					float dW2 <- wsA - (nb2.z + nb2.water_h);
					if (dW2 > EPS_HEAD) {
						if (dW2 > max_dW) { max_dW <- dW2; }
						float heff2 <- wsA - zmax2;
						if (heff2 > EPS_WET) {
							float v2 <- min((heff2 ^ 1.6667) * sqrt(dW2) * k2 * dts, dW2 * cap_vol);
							if (v2 > 0.0) { out2 <- v2; totV <- totV + v2; }
						}
					}
				}
				if (nb_open > 3) {
					float dW3 <- wsA - (nb3.z + nb3.water_h);
					if (dW3 > EPS_HEAD) {
						if (dW3 > max_dW) { max_dW <- dW3; }
						float heff3 <- wsA - zmax3;
						if (heff3 > EPS_WET) {
							float v3 <- min((heff3 ^ 1.6667) * sqrt(dW3) * k3 * dts, dW3 * cap_vol);
							if (v3 > 0.0) { out3 <- v3; totV <- totV + v3; }
						}
					}
				}
			}
			if (totV > 0.0) {
				...
				if (totV > availV) {
					...
					out0 <- out0 * sc; out1 <- out1 * sc; out2 <- out2 * sc; out3 <- out3 * sc;
				}
				has_out <- true;
				idle_subs <- 0;
			} else if (!idle_hr and idle_sleep) {
