/**
* Name: Hanoi FastFlood Model LISEM OpenLISEM Optimized V2
* Author: Thành Đô Nguyễn
*
* V2 of the OPTIMIZED variant of "Hanoi FastFlood Model LISEM OpenLISEM.gaml".
* On top of O1-O16 of the Optimized model (kept unchanged, listed below), V2
* adds one mass-conservation fix and three exact optimizations:
*
*   O17 [fix]   Stale-flux leak of O16 closed: a cell that exported flux in
*       the LAST relaxation iteration of an hour and dried below h_eps in
*       that same iteration kept its nonzero q_* if its block was deactivated
*       at the next hourly recompute (pass 1, which resets q via had_q, no
*       longer visits it). An active neighbour across the block boundary
*       then gathered that stale flux every iteration - phantom water
*       (~0.1 mm/iter, self-limiting but a conservation violation the
*       non-skipping models cannot produce). V2 clears q_had_q of dry
*       cells during the hourly wetness scan, restoring the invariant
*       "cell outside the active set => q = 0".
*   O18 [exact] All remaining full-grid passes outside the relaxation loop
*       (the bookkeeping ask, the wet-cell scan, the u_peak maximum, and the
*       hourly block-wetness recompute) are restricted to the active lists
*       when block skipping is on. Exact because a cell outside the active
*       set provably has h <= h_eps: it cannot be flooded, cannot change
*       color, and its arrival_h/h_peak/u_peak are frozen (the domain
*       maximum is folded as max(previous, max over active)). On quiet hours
*       (pre-breach week, late recession) these scans dominated the cycle.
*       Falls back to full-grid scans for one hour whenever block skipping
*       was off in the previous cycle (stale active lists are never read).
*   O19 [exact] rebuild_active_lists no longer concatenates with list '+'
*       (which copies the accumulated list per block, O(blocks x length));
*       it appends in place with '<<+'.
*   O20 [exact] The flux split divides once per cell (q_norm = courant*h /
*       w_tot) instead of once per direction: up to 3 divisions per wet cell
*       per iteration saved in the hottest loop (FP reassociation only).
*
* Optimizations inherited from the Optimized model
* (those marked [exact] are mathematically identical to the original):
*
...
*/
model HanoiFastFloodLISEMOpenLISEMOptimizedV2
