/**
* Name: Hanoi FastBathtub Flood Model 1926
* Author: Thành Đô Nguyễn (2026-06-13)
*
* GOAL: the SPEED of the FastFlood / LISEM relaxation model
*       ("Hanoi FastFlood Model LISEM OpenLISEM Optimized V5.gaml")
*       with the FLOOD EXTENT of the agent-based bathtub model
*       ("Hanoi ABM Flood Model 1926.gaml").
*
* WHY A DIFFERENT ENGINE.
*   The two parent models converge to two different equilibria:
*     - the ABM solves a STORAGE equilibrium - the weir gate fills the plain
*       until the water level reaches the river head, mass is conserved and the
*       domain edge is closed, so the attractor is a flat pool at the river
*       stage over every breach-connected cell below stage, bounded by the
*       dykes and the internal saddles (a connectivity-constrained BATHTUB);
*     - FastFlood / AccuFluxDiffusive solves a FLUX equilibrium (Qin = Qout) and
*       relaxes to a sloping THROUGHFLOW sheet that thins below threshold away
*       from the breach, leaks at the free-draining domain edge, and stops on a
*       grid-mean convergence test. A flood ponding BEHIND dykes is
*       storage-dominated, so FastFlood is the wrong fast surrogate here and no
*       amount of tuning closes the gap.
*
* THE FAST + ACCURATE ENGINE (SPREAD-limited, connectivity-constrained,
* hysteretic level-pool). Two static fields are precomputed per cell (by one
* fast-sweeping pass, recomputed only when a breach opens, ~3 times total):
*   S[c] = SPILL LEVEL: the lowest water level at which c first connects to a
*          breach - the minimax (least-highest-sill) path level from the breach
*          corridor cells, with non-breached dykes and the river as barriers
*          (the ABM "water enters only through a breach" rule).
*   D[c] = FRONT DISTANCE: the geodesic distance (m) from the nearest breach
*          corridor through passable terrain - how far the flood must travel.
*
*   Then each hour, at river stage L and time t, a cell is wetted iff BOTH the
*   flood front has reached it AND it is river-connected at the stage:
*       D[c] <= front_celerity * (t - t_breach)   (spread limit)  AND
*       S[c] <= L                                  (level limit)
*       -> h = L - z   (tracks the stage up AND down; drains back through the breach)
*   A cell that later fails the level test (S > L on the receding limb) keeps its
*   water -> trapped casier. The SPREAD limit is the key: it makes the extent
*   spread-limited like the ABM (not a full-equilibrium bathtub), and DATUM-ROBUST
*   - geodesic distance is invariant to a uniform vertical shift of the DEM, so
*   the footprint barely changes between the 40 m and 50 m DEMs. front_celerity is
*   the single knob to CALIBRATE against the ABM peak area; front_limit = false
*   recovers the pure (datum-sensitive, over-predicting) level-pool bathtub.
*   The whole dynamic solver is still one filtered parallel ask per hour, no
*   iteration -> LISEM-class speed.
*
* Inputs (../includes/): DEM selectable via the "DEM file" parameter; default
*   mnt-gz40.asc = the ARCHIVAL 1926-datum MNT the ABM runs on (z 1.9..16.1 m,
*   mean 6.75 m), so the two models are directly comparable. mnt-gz50.asc is the
*   modern DEM used by the V5 LISEM model (different vertical datum). Plus
*   RedRiver1925.shp, Buildings1925.shp, Lakes1925.shp, 5_arrival_time.shp,
*   Dyke_vip.shp (BREAK/DATE/Commune/HOUR), WaterDischarge.csv.
*
* Forcing & breach scenario are kept identical to the V5 LISEM model: observed
*   daily discharge -> Manning rating curve -> stage (peak 11.93 m, Gourou
*   fig. 9), and the Dyke_vip.shp 28/29-07 breach dates+HOUR cut as a corridor.
*/
model HanoiFastBathtubFlood1926
...
global {
	...
	float step <- 1 #h;                       // ONE CYCLE = ONE HOUR (as the V5 model)

	// ------------------------------------------------------------------ river stage forcing
	// (identical to the V5 LISEM model: observed discharge -> rating curve -> stage)
	...
	// ------------------------------------------------------------------ breach scenario
	// (identical geometry to the V5 LISEM model: invert from the lowest
	...
	// datum-robust: geodesic distance through the terrain is invariant to a
	// uniform vertical shift of the DEM, so the footprint barely changes between
	// the 40 m and 50 m DEMs - unlike a pure level-pool.
	// CALIBRATION: this is the one knob to fit to your ABM. Run, read the peak
	// flooded area, and raise/lower front_celerity until it matches the ABM peak
	// on EITHER DEM (it should then match on both). Larger -> faster spread ->
	...
	//   S[c]  (spill_lvl) = lowest stage at which c connects to a breach: the
	//         minimax (least-highest-sill) path level from the corridor seeds.
	//   D[c]  (front_dist) = geodesic distance (m) from the nearest breach
	//         corridor through passable terrain - how far the flood must travel
	//         to reach c. Combined with the front celerity this gives the
	//         spread-limited, datum-robust footprint.
	...
	// filtered parallel ask per hour, no iteration. A cell is wetted when BOTH:
	//   (i)  the flood front has arrived:   front_arrival[c] <= time since sim start
	//        (front_arrival = the cell's own breach open time + travel time, so
	//        each breach spreads from when IT opens - no burst at later breaches)
	//   (ii) it is river-connected at the stage: S[c] <= stage
	// then it tracks the stage up and down (drains back through the breach).
	// Limit (i) makes the extent spread-limited like the ABM and datum-robust
	// (geodesic travel time is invariant to a uniform DEM shift); limit (ii)
	// keeps depths physical and bounds the pool by the stage. Cells that fall
	// out of (ii) keep their water -> trapped casiers on the receding limb.
...
species dyke schedules: [] {
	...
	// the cut cells become a passable entry, then the spill-level field is
	// rebuilt (scenario definition, identical to the V5 LISEM model)
	action open_breach {
		opened <- true;
...
experiment fastbathtub_1926_fast type: gui {
