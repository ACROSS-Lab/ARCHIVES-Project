/**
* Name: Hanoi ABM Flood Model 1926 (V3-optimized)
* Description:
...
*     A) flow_step phase B iterates `flow_list` under an `if (has_out)` guard instead of building a
*        fresh `flow_list where each.has_out` sub-list every sub-step - one fewer heap allocation per
*        sub-step; idle / non-donor cells (has_out = false) skip the body, identical result.
*     B) flow_step phase A caches each neighbour's `z` (nbz) instead of reading nb.z twice per face.
...
*     D) flow_step phase A also uses an `if (!idle_hr)` guard instead of allocating
...
global {
	...
	action flow_step (float dts) {
		...
		// (iterate flow_list under an if-guard instead of allocating a `where !idle_hr` copy)
		ask flow_list parallel: true {
			...
			if (!idle_hr and nb_open > 0 and water_h > EPS_WET) {
				float wsA <- z + water_h;
		...
		// (iterate flow_list under an if-guard instead of allocating a `where each.has_out` copy)
		ask flow_list {
			if (has_out) {
				float tot <- 0.0;
				loop i from: 0 to: nb_open - 1 {
					...
					if (v > 0.0) {
						...
						tot <- tot + v;
						if (!nb.stirred) { nb.stirred <- true; stirred_cells << nb; }
						nb.idle_hr <- false;
						nb.idle_subs <- 0;
				...
				if (!stirred) { stirred <- true; stirred_cells << self; }
			}
		}
	...
	reflex hourly_tick when: current_date >= next_tick {
		...
		ask building {
			...
			if (enable_visuals) { color <- status = 2 ? #red : (status = 1 ? #orange : rgb(120, 120, 125)); }
		}
