/**
* Name: Hanoi FastFlood Model LISEM OpenLISEM Optimized V3
* Author: Thành Đô Nguyễn
*
* V3 of the OPTIMIZED variant of "Hanoi FastFlood Model LISEM OpenLISEM.gaml".
* On top of O1-O20 of Optimized / Optimized V2 (kept unchanged, summarized
* below), V3 adds four optimizations:
*
*   O21 [exact at convergence] River cells removed from relaxation pass 2.
*       In forcing mode the pass-2 river branch (h <- h_forced) is a no-op
*       after the first iteration: nothing else writes river h within the
*       hour (pass 1 writes only q_*, breach corridors exclude river cells,
*       h_forced and z_dyn are constant between hourly steps). V3 therefore
*       commits river h and wsl ONCE at the start of each hourly step and
*       runs pass 2 on land cells only (per-block land member lists). The
*       land pass-2 body also drops the per-cell relax_force_river/is_river
*       test and the relax_source_m term (always 0 in forcing mode); the
*       static refinement keeps the full-grid body with the rain source.
*       River dh_abs stays exactly 0.0 (the value the V2 river branch wrote
*       on every tracked iteration), so convergence sums are unchanged.
*       Only transient difference: iteration 1 of each hour sees the new
*       stage immediately instead of one iteration late - this matches the
*       C++ ordering (AccuFluxDiffusive sets H from HForced BEFORE its
*       iteration loop) and is identical at the converged state that each
*       hourly step commits.
*   O22 [exact] schedules: [] on the grid and on every species: none of them
*       has a reflex (all behavior is driven by global reflexes and explicit
*       asks), yet GAMA's scheduler stepped all ~37k agents every cycle.
*       Scheduling is orthogonal to displays, aspects, spatial queries and
*       asks, so nothing else changes.
*   O23 [exact] The fast-sweeping inner loop used min([...]) over a freshly
*       allocated 4-element list per cell visit (init only); replaced with
*       nested binary min - same value, no allocation.
*   O24 [exact] block_size default 16 -> 8: the active set hugs the flood
*       front tighter (a front block contributes up to 256 dry cells at
*       16x16, 64 at 8x8). Block skipping is exact for ANY block size (4-dir
*       flux travels at most one cell per iteration and the active set is
*       wet blocks + all neighbour blocks), so this is pure tuning; the
*       block size is also exposed as a parameter for experiments.
*
* Inherited optimizations (full details in the Optimized / Optimized V2
...
* Dykes.shp records 28/29-07 (kept as the data-driven choice). peak_stage
* keeps the calibrated 11.93 m (V2 value): the DEM crests (max 16.1 m) are
* not overtopped, so the flood enters through the breaches only -
* historically correct.
*/
model HanoiFastFloodLISEMOpenLISEMOptimizedV3
...
global {
	...
	list<cell> active_interior_land <- []; // O21: active interior minus river (pass 2)
	list<cell> active_boundary_land <- []; // O21: active boundary minus river (pass 2)
	...
	init {
		write "=== Hanoi FastFlood (LISEM / OpenLISEM port, OPTIMIZED V3) ===";
	...
	action rebuild_active_lists {
		...
		loop b over: act {
			...
			active_interior <<+ b.members_interior;
			active_boundary <<+ b.members_boundary;
		...
		ask p_int parallel: true {
			if h > h_eps {
				...
				if w_tot > 0.0 {
					...
					q_e <- w_e > 0.0 ? min(q_norm * w_e, stab_factor * dz_e) : 0.0;
					q_w <- w_w > 0.0 ? min(q_norm * w_w, stab_factor * dz_w) : 0.0;
					q_n <- w_n > 0.0 ? min(q_norm * w_n, stab_factor * dz_n) : 0.0;
					q_s <- w_s > 0.0 ? min(q_norm * w_s, stab_factor * dz_s) : 0.0;
					had_q <- true;
				} else if had_q { // O15
	...
	reflex fastflood_relax {
		...
		if block_skipping {
			...
			loop ce over: (blocks_in_use ? active_all : all_cells) {
				if ce.h > h_eps {
					ce.my_block.wet <- true;
				} else if ce.had_q {
					ce.q_e <- 0.0; ce.q_w <- 0.0; ce.q_n <- 0.0; ce.q_s <- 0.0;
					ce.had_q <- false;
				}
			}
		...
		loop it from: 1 to: n_max {
			...
			if track_dh {
				...
				// (river cells keep dh_abs = 0, as the V2 river branch wrote)
